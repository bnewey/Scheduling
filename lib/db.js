...
//gets the machine table names from mysql on db.js require (Immediately Invoked Function)
var getMachines = (function(){
    pool.query('Select id,machine_table_name from machines', function (error,results) {
        if(error){
            console.log(error);
            return;
        }        
        let results_to_array = JSON.parse(JSON.stringify(results));
        
        //Put results into an array arranged by id
        results_to_array.forEach((item, index) =>{
            if(item.hasOwnProperty("id") && item.hasOwnProperty("machine_table_name") ){
                db.machine_array[item.id-1] = item.machine_table_name;
            }
        });
        return;
    });
