...
const Search = function(props) {
  ...
  const searchTableObject= [
    {value: "fpoi.sales_order_id", displayValue: 'Sales Order #'},
    {value: "fpoi.record_id", displayValue: 'FP Item #'},
    {value: "fpoi.model", displayValue: 'Model'},
    {value: "wo.record_id", displayValue: 'Work Order #'},
    {value: "en.name", displayValue: 'Job Name'},
  ];
  ...
  useEffect(() => {
    if(searchTable == null){
      var tmp = window.localStorage.getItem('searchPOTable');
      var tmpParsed;
      if(tmp){
      ...
      if(tmpParsed){
        ...
        setSearchTable("en.name");
      }
    }
    if(searchTable){
      window.localStorage.setItem('searchPOTable', JSON.stringify(searchTable));
    }
  ...
  useEffect(() => {
    if(searchHistory == null){
      var tmp = window.localStorage.getItem('searchPOHistory');
      var tmpParsed;
      if(tmp){
        tmpParsed = JSON.parse(tmp);
      }
    ...
    if(Array.isArray(searchHistory)){
      window.localStorage.setItem('searchPOHistory', JSON.stringify(searchHistory));
    }
  ...
  const search =(searchTable, searchValue)=>{
    return new Promise((resolve,reject)=>{
      ...
      WorkOrderDetail.searchAllFPOrderItems(searchTable, searchValue)
      .then((data)=>{
        if(data){
          //console.log(data);
  ...
  const handleEnterSearch = async (keyCode, event)=>{
    ...
    if(keyCode === 13 && id ==  "po_search_input"){ //enter key & input element's id
      try {
  ...
  const handleSearchClick = async() =>{
    if(searchOpen == false){
      ...
      //submit search
      setPurchaseOrders(await search(searchTable, searchValue));
    }
  }
            ...
            <Autocomplete
              ...
              onInputChange={async(event, value, reason)=> {
                ...
                if(matchValue){
                  ...
                  setPurchaseOrders(await search(searchMatch.searchTable, matchValue ));
                }
              } }
              renderInput={(params) => {  searchRef.current = params.inputProps.ref.current; return (<TextField
                ...
                inputProps={{ 'aria-label': 'search work orders', id: "po_search_input", ref: searchRef, autoFocus: true}}
                autoFocus={true}
                {...params}
