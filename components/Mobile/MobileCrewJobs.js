...
const MobileCrewJobs = ({ user }) => {
  ...
  const formatDateOnly = (dateStr) => {
    if (!dateStr) return "";
    return dateStr.split(" ")[0]; // drop the time
  };
          ...
          .filter((job) => {
            const dateStr =
              job.job_type === "install"
                ? job.sch_install_date
                : job.drill_date;
            if (!dateStr) return false;
            const jobDate = new Date(dateStr);
            if (isNaN(jobDate.getTime())) return false;
            return jobDate >= now && jobDate <= threeMonthsOut;
          });
  ...
  const handleCompleteJob = async () => {
    ...
    try {
      const ok = await Crew.updateCrewJobCompleted(1, confirmJob.id, confirmJob.crew_id, user);
      if (ok) {
        ...
        {loadingJobs ? (
          ...
          jobs && jobs.length ? (
            ...
            <Typography variant="body2" style={{ marginTop: "1rem" }}>
              No jobs in the next 3 months for this crew member.
            </Typography>
          )
        ) : (
          <Typography variant="body2" style={{ marginTop: "1rem" }}>
