...
const Search = function(props) {
  ...
  const { ordersOut, setOrdersOut, ordersOutSearchRefetch, setOrdersOutSearchRefetch, currentView, setCurrentView, views} = useContext(ListContext);
  const searchOpen = currentView && currentView.value == "ordersOutSearch";

  const searchTableObject= [
    {value: "all", displayValue: 'All'},
    {value: "s.id", displayValue: 'ID#'},
  ...
  useEffect(()=>{
    if(ordersOutSearchRefetch){
      setOrdersOutSearchRefetch(false);
  ...
  },[ordersOutSearchRefetch])

  //Save and/or Fetch searchTable to local storage
  useEffect(() => {
    if(searchTable == null){
      var tmp = window.localStorage.getItem('searchInvOrdersOutTable');
      var tmpParsed;
      if(tmp){
    ...
    if(searchTable){
      window.localStorage.setItem('searchInvOrdersOutTable', JSON.stringify(searchTable));
    }
  ...
  useEffect(() => {
    if(searchHistory == null){
      var tmp = window.localStorage.getItem('searchInvOrdersOutHistory');
      var tmpParsed;
      if(tmp){
        tmpParsed = JSON.parse(tmp);
      }
    ...
    if(Array.isArray(searchHistory)){
      window.localStorage.setItem('searchInvOrdersOutHistory', JSON.stringify(searchHistory));
    }
  ...
  const search =(searchTable, searchValue)=>{
    return new Promise((resolve,reject)=>{
      ...
      if(searchTable === "all"){
        InventoryOrdersOut.superSearchAllOrdersOut(searchTableObject.filter((j)=>j.value !== "all").map((v)=> v.value), searchValue)
          .then((data)=>{
            if(data){
          ...
          .catch((error)=>{
            cogoToast.error("Failed to search all tables within ordersOut");
            reject(error);
            
          })
      }else{
        InventoryOrdersOut.searchAllOrdersOut(searchTable, searchValue)
        .then((data)=>{
          if(data){
            //console.log(data);
            //Update search history
        ...
        .catch((error)=>{
          cogoToast.error("Failed to search ordersOut");
          reject(error);
  ...
  const handleSearchClick = async() =>{
    if(searchOpen == false){
      setCurrentView( views.filter((view)=> view.value == "ordersOutSearch")[0] )
      
    }else{
      //submit search
      setOrdersOut(await search(searchTable, searchValue));
    }
  }
            ...
            <Autocomplete
              ...
              onInputChange={async(event, value, reason)=> {
                ...
                if(matchValue){
                  ...
                  setOrdersOut(await search(searchMatch.searchTable, matchValue ));
                }
              } }
              renderInput={(params) => {  searchRef.current = params.inputProps.ref.current; return (<TextField
                
                className={classes.input}                
                placeholder="Search OrdersOut"
                inputProps={{ 'aria-label': 'search ordersOut', id: "ordersOut_search_input", ref: searchRef, autoFocus: true}}
                autoFocus={true}
                {...params}
