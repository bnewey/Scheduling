...
import TaskListFilterSaveDialog from './SignListFilterSaveDialog'
...
const SignListFilter = (props) => {
    //PROPS
    const { filteredItems, setFilteredItems } = props;

    const { signsSaved,filterInOrOut,setFilterInOrOut,filterAndOr, setFilterAndOr, filters, setFilters, user} = useContext(ListContext);
    ...
    const [tableConfig, setTableInfo] = useState([
        { field: 'install_date', text: 'Install Date', type: 'date' },
        { field: 'type', text: 'WO Type', type: 'text'},
        { field: 'state', text: 'Ship Group', type:'text' },
        { field: 'work_order', text: 'WO#', type: 'number'   },
        { field: 'product_to', text: 'Product Goes To', type: 'text'}
    ]);
    ...
    useEffect(() => {
        if(filterInOrOut == null){
            var tmp = window.localStorage.getItem('signfilterInOrOut');
            var tmpParsed;
        ...
        if((filterInOrOut)){
            window.localStorage.setItem('signfilterInOrOut', JSON.stringify(filterInOrOut));
        }
    ...
    useEffect(() => {
        if(filterAndOr == null){
            var tmp = window.localStorage.getItem('signfilterAndOr');
            var tmpParsed;
            if(tmp && tmp != undefined){
                tmpParsed = JSON.parse(tmp);
            }
        ...
        if((filterAndOr)){
            window.localStorage.setItem('signfilterAndOr', JSON.stringify(filterAndOr));
        }
